...
# Creating shims for those binaries will prevent chainenv users to run those
...
conda_exists() {
  ...
  local condas=($(echo "${CHAINENV_ROOT}/versions/"*"/bin/conda" "${CHAINENV_ROOT}/versions/"*"/envs/"*"/bin/conda"))
...
# override `make_shims` to avoid conflict between chainenv-virtualenv's `envs.bash`
# https://github.com/chainenv/chainenv-virtualenv/blob/v20160716/etc/chainenv.d/rehash/envs.bash
make_shims() {
  local file shim
  for file do
    shim="${file##*/}"
